...
df_feat = df.select('V3', 'V4', 'V10', 'V11', 'V12', 'V14', 'Class')
# Print Schema
df_feat.printSchema()

# Select fraud and non-fraud transactions and limit non-fraud transactions to the same number as fraud transactions
fraud_df = df_feat.filter(f.col('Class') == 1)
non_fraud_df = df_feat.filter(f.col('Class') == 0).limit(fraud_df.count())
...
feature_columns = [col for col in df_feat.columns if col != 'Class']
